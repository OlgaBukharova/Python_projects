...
def score(a): # счет на голубом фоне (для текущего счета)
    rect(screen, (0,255,255), (300,300,600,200))
    S = Number([300, 300, 600, 200], a)
    S.draw()

def final_score(f_a): # счет на желтом фоне в розовой рамке (для финального счета)
...
while not finished:
    ...
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN:
            for y in range (len(A)): # после нажатия перебирает все шары
                ...
                if (event.pos[0]<=bb.coord[0]+bb.r) and (event.pos[0]>=bb.coord[0]-bb.r) and (event.pos[1]<=bb.coord[1]+bb.r) and (event.pos[1]>=bb.coord[1]-bb.r): # условие попадания по шару
                    bb.r=0 # сужение шара
                    number_popped += 1 # увеличение количества лопнувших шаров
                    '''прогрессивная шкала получения очков''' #  основа: мгновенная скорость (среднее квадратическое скоростей по осям), размер
                    if bb.color == RED: # за маленький шар в три раза больше (мгновенная скорость * 3)
                        result += math.ceil(math.sqrt((bb.velocity[0])**2+(bb.velocity[1])**2)*3) 
                    if bb.color == GREEN: # за средний шар в два раза больше (мгновенная скорость * 2)
                        result += math.ceil(math.sqrt((bb.velocity[0])**2+(bb.velocity[1])**2)*2)
                    if bb.color == BLUE: # за большой шар очки не увеличиваются (мгновенная скорость * 1)
                        result += math.ceil(math.sqrt((bb.velocity[0])**2+(bb.velocity[1])**2))
        if  len(A) - number_popped >= check: # при достижении контрольной цифры шаров на экране
            ...
            file=open('game_balls_score_file.txt','a') # результат записывается в файл
    ...
    if (n_new % 100 == 0): # кажде 100 обновление экрана добавляет шар
        add()
