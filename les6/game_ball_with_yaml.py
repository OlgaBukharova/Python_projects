...
''' имя игрока и результат записывается в файл '''
def write_file(z, x):
    import yaml
    template = '''
    results:
        ...
        - points: x
    '''
    with open("results.yaml", 'r') as y_file:
        loaded = yaml.load(y_file, Loader = yaml.Loader)
    loaded['results'].append({'name': z, 'points': str(x)})
    with open("results.yaml", 'w') as y_file:
        yaml.dump(loaded, y_file)
...
while not finished:
    ...
    for event in pygame.event.get():
        ...
        if  len(A) - number_popped >= check: # при достижении контрольной цифры шаров на экране
            ...
            write_file(user_name, result) # записть в файл
            finished = True
